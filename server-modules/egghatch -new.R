EggHatch <- function(ggplot2_plot, cond.list, width_Man = NULL,input_size=c(.4,.4,.4,.4),xpos,inputtext){
 # tryCatch({
# use this for Relevel
    library(Epi)
    # default relevel to avoid index issue
  #print(ggplot2_plot$data$Variable)
   # ggplot2_plot$data$xmin = Relevel(ggplot2_plot$data$xmin, ref = c(ggplot2_plot$data$xmin))
    ...
    # if statement for when no value input in 'fill'
      ...
      #xpos<-ggplot2_plot$layers[[1]]$data$x
      print(xpos)
      ...
      two_percent_of_largest_mean<-((max(unlist(lapply(ggplot2_plot$data$val,FUN=max)))/100)*2)
      ...
      for (pos in 1:length(df$Condition)){
        ...
        if(grep("A|B|C|D", df$Pattern[pos], ignore.case=TRUE)){
          ...
          if ((grepl('A', df$Pattern[pos], ignore.case=TRUE))){
            ...
            vert_bar_count =(((ggplot2_plot$layers[[1]]$geom_params$width[pos])/100)*10)
            Condition_Pos<-as.numeric(df$Condition[pos])
 
            for (i in 1){
 
              if (is.null(fill_value[pos])){
 
                BAR_OUTPUT<-BAR_OUTPUT+geom_bar(data=BAR_OUTPUT$data[Condition_Pos,], position=position_dodge2(input_position_dodge,padding=0), stat="identity", colour=input_colour, size=input_size, width=(ggplot2_plot$layers[[1]]$geom_params$width[pos]))
              }
 
              if (!is.null(fill_value[pos])){
 
              BAR_OUTPUT<-BAR_OUTPUT+geom_bar(data=BAR_OUTPUT$data[Condition_Pos,], position=position_dodge2(input_position_dodge,padding=0), stat="identity", colour=input_colour, size=input_size, width=(ggplot2_plot$layers[[1]]$geom_params$width[pos]),fill=fill_value[pos])
...
#                                      BAR_OUTPUT<-BAR_OUTPUT+geom_bar(data=BAR_OUTPUT$data[Condition_Pos,], position=position_dodge(input_position_dodge), stat="identity", colour=input_colour, size=input_size, width=(ggplot2_plot$layers[[1]]$geom_params$width[pos])-4/19, fill='transparent')
                             
                 BAR_OUTPUT = BAR_OUTPUT + geom_abline(aes(xintercept=seq(xpos[pos],xpos[pos]),))
                vert_bar_count = vert_bar_count + (((ggplot2_plot$layers[[1]]$geom_params$width[pos])/100)*10)#((ggplot2_plot$layers[[1]]$geom_params$width)/4)
   ...
   # ensure the Condition_Position value is correct here
            ...
            if ((grepl('B', df$Pattern[pos], ignore.case=TRUE))){  
              ...
              point_1_percent_of_current_mean <- ((BAR_OUTPUT$data$val[Condition_Pos]/100)*.1)                   
              while((BAR_OUTPUT$data$val[Condition_Pos])>=two_percent_of_largest_mean){ 
 
                # this breaks while loop to prevent going below x-axis 0
                if(((as.numeric(BAR_OUTPUT$data$val[Condition_Pos]))-(ggplot2_plot$data$val[Condition_Pos]/100))<0){
 
                  break
 
                }
 
                BAR_OUTPUT$data$val[Condition_Pos]<-(as.numeric(BAR_OUTPUT$data$val[Condition_Pos]))-two_percent_of_largest_mean
 
                # This stops line very close to 0 on y-axis
                if(((BAR_OUTPUT$data$val[Condition_Pos])<((two_percent_of_largest_mean/100)*15))){
 
                  break
 
                }
 
                BAR_OUTPUT<-BAR_OUTPUT+geom_bar(data=BAR_OUTPUT$data[Condition_Pos,], position=position_dodge2(input_position_dodge,padding=0), stat="identity", colour=input_colour, size=input_size, width = ggplot2_plot$layers[[1]]$geom_params$width[pos], fill='transparent')
            ...
            if ((grepl('C', df$Pattern[pos], ignore.case=TRUE))){
              ...
              # Code for diagonal C
              position_input_x<-as.numeric(xpos[pos])
              position_input_y<-as.numeric(xpos[pos])
              print(position_input_x)
              # half of input width
              # when minused from centre of bar point on x-axis
              # it gives value at start of bar on x-axis
              # inverse true for end of bar width
              half_input_width<-input_width[pos]/2
              ten_perc_input_width<-((input_width[pos]/100)*10)
              ...
              y_axis_max<- ggplot2_plot$data$val[Condition_Pos]
              ...
              for(i in 1:19){
                ...
                if (df_diag$x[2]<(as.character(x_axis_max))){
                  ...
                  BAR_OUTPUT<-BAR_OUTPUT+geom_path(data=df_diag, aes(x=x, y=y),colour = "black",size=1)
            ...
            if ((grepl('D', df$Pattern[pos], ignore.case=TRUE))){
              ...
              df_diag <- data.frame(                                  # start on x-axis-end on x-axis                
                x = c(x_axis_max,x_axis_max),                                                   # start on y-axis-end on y-axis                 
                y = c(y_axis_max,y_axis_max), Fill='Hope you\'re having a nice day.') 
              
              for(i in 1:19){                                  # run if 1st x-position is less or equal to leftmost edge of bar                 # minus 10% of total input width                
          ...
          if ((!grepl("[^B]", df$Pattern[pos], ignore.case=TRUE))==TRUE){
            ...
            Condition_Pos<-as.numeric(df$Condition[pos])
            one_percent_horiz<-(BAR_OUTPUT$data$val[Condition_Pos])/100
            Twenty_percent_horiz<-(one_percent_horiz*20)
 
            point_1_percent_of_current_mean <- ((BAR_OUTPUT$data$val[Condition_Pos]/100)*.1)                          # draw original bar -> avoids any issue with 'fill' and drawing incorrect height
            BAR_OUTPUT<-BAR_OUTPUT+geom_bar(data=BAR_OUTPUT$data[Condition_Pos,], position=position_dodge2(input_position_dodge,padding=0), stat="identity", colour=input_colour, size=input_size, width = ggplot2_plot$layers[[1]]$geom_params$width[pos])                          # drawing lines (via minusing mean) above 0 on y-axis             
            while((BAR_OUTPUT$data$val[Condition_Pos])>=two_percent_of_largest_mean){ 
...
print(y_axis_max)
             ...
             for(i in 1:19){
               print(df_diag$x[2]<(as.character(x_axis_max)))
                ...
                if (df_diag$x[2]<(as.character(x_axis_max))){
                  ...
                  # draw using current df_diag values
                  BAR_OUTPUT<-BAR_OUTPUT+geom_path(data=df_diag, aes(x=x, y=y),colour = input_colour,size=1)
            ...
            if ((grepl('D', df$Pattern[pos], ignore.case=TRUE))){
              ...
              y_axis_max<- ggplot2_plot$data$val[position_input_y]
              # need to pull the max value from the example_plot the max boundries...
              # or caluclate the
              z_percent_of_current_mean<-(y_axis_max/100)*10
