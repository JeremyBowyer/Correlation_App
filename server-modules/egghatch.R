        ...
        if(grep("A|B|C|D", df$Pattern[pos], ignore.case=TRUE)){
          ...
          if ((grepl('A', df$Pattern[pos], ignore.case=TRUE))){
            ...
            for (i in 1){
              ...
              # loop 4 times
              for (i in 1:((ggplot2_plot$layers[[1]]$geom_params$width[pos])/vert_bar_count)){
                ...
                BAR_OUTPUT<-BAR_OUTPUT+geom_bar(data=BAR_OUTPUT$data[Condition_Pos,], position=position_dodge(input_position_dodge), stat="identity", colour=input_colour, size=input_size, width=(ggplot2_plot$layers[[1]]$geom_params$width[pos])-vert_bar_count, fill='transparent')
          ...
          if ((!grepl("[^B]", df$Pattern[pos], ignore.case=TRUE))==TRUE){
            ...
            BAR_OUTPUT<-BAR_OUTPUT+geom_bar(data=BAR_OUTPUT$data[Condition_Pos,], position=position_dodge(input_position_dodge), stat="identity", colour=input_colour, size=input_size, width = ggplot2_plot$layers[[1]]$geom_params$width)                          # drawing lines (via minusing mean) above 0 on y-axis             
            while((BAR_OUTPUT$data$val[Condition_Pos])>=two_percent_of_largest_mean){ 
